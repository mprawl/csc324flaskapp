	...
	stages {
		...
		stage('Build Docker Image')
				...
				sh 'sudo chmod 666 /var/run/docker.sock'
                                sh 'docker build -t mprawl/flaskdemo:$VERSION .'
                                sh 'docker build -t mprawl/flaskdemo:latest .'
                        }
                }
		...
		stage('Push Image to Dockerhub') {
			...
			steps {
				sh 'docker push mprawl/flaskdemo:$VERSION'
				sh 'docker push mprawl/flaskdemo:latest'
			}
		}
